...
class Expense {
  ...
  static upload() async {
    List<Expense> _listExpense = await ExpenseDAO().getToUpload();
    ...
    for (var _expense in _listExpense) {
      requestData = _expense.toMap();
      ...
      if (response.statusCode == 200) {
        if (_expense.id == null) {
          ...
          _expense.id = jsonData["id"];
        }

        ExpenseDAO().update(_expense, isModified: false);
      }
    }
  }
  ...
  static download() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var jsonExpense in jsonData) {
        expense = Expense.fromMap(jsonExpense);

        bool ok = false;

        Expense existExpense = await ExpenseDAO().getByID(expense.id);

        if (existExpense != null) {
          expense.mobID = existExpense.mobID;
          ok = await ExpenseDAO().update(expense, isModified: false);
        } else {
          int mobID = await ExpenseDAO().insert(expense, isModified: false);
        ...
        if (ok) {
          String urlProcessed =
              'http://$_serverIP/api/paydesk/processed?from=mobile&id=${expense.id.toString()}';
