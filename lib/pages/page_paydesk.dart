              ...
              child: Scaffold(
                body: LiquidPullToRefresh(
                    ...
                    child: _id == 0
                      ...
                      child: Text('Записiв не зайдено',
                        style: TextStyle(fontSize: 20.0),),)],)
                        : ListView.builder(
                      itemCount: _id,
                      itemBuilder: (BuildContext context, int index) {
                        if(_id != _paymentArray.length) return Center(
                        ...
                        return InkWell(
                            onTap:(){
                              Navigator.push(context, MaterialPageRoute(
                                  builder: (_){
                                    return WorkWithPayment(edit: false,
                                      index: index, current: _paymentArray[index],);
                                  })).whenComplete(() => _load());},
                            child: Hero(
                                tag: 'paydesk_$index',
                                child: Material(
                                  ...
                                  child: ListTile(
                                    ...
                                    leading: CircleAvatar(
                                        child: Text(_paymentArray[index].id.toString())),
                                    ...
                                    subtitle: Column(
                                      ...
                                      children: <Widget>[
                                        ...
                                        _paymentArray[index].date == null
                                            ...
                                            .format(_paymentArray[index].date)}'),
  ...
  Future<void> _handleRefresh(){
    ...
    return completer.future.then<void>((_){
      _scaffoldKey.currentState?.showSnackBar(SnackBar(
        ...
        content: const Text('Сторiнка оновлена',
...
class _WorkWithPaymentState extends State<WorkWithPayment> {
  ...
  _WorkWithPaymentState(this._editable,
    ...
    if(!_editable){
      ...
      _enableEdit = false;
      current.files != null
          ? _files = _takeFilesFromPath(current)
          : File('');
      _index = index;
      _icon = Icons.edit;
  ...
  List<IconData> _icons = const [Icons.image,
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed:()async{
          ...
          } else if(_enableEdit && !_editable && _formKey.currentState.validate()){
  ...
  Widget _setAppBar(){
    if(_editable) return AppBar(title: Text('Додати платiж'),);
    return null;
  }
  ...
  Widget _setStatus(){
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          Opacity(
              child: Icon(FontAwesomeIcons.userClock, size: 18,),opacity: 0.5),
          Container(
            margin: EdgeInsets.only(left: 16.0),
            child: Text('Статус: $_statusText',
  ...
  Widget _chooseImageOrPDF(File file){
    if(path.extension(file.path)=='.pdf') return Column(
      children: <Widget>[
        Image.asset('assets/pdf.png', width: 100,),
  ...
  bool _isNotCorrectAmount(String value){ //Check if the sum is of type *.xx
    ...
    try {
      Pay pay = Pay(
        ...
        date: _dateController.text.isNotEmpty
            ? DateTime.parse(formatDate(DateFormat("dd-MM-yyyy")
  ...
  Future<bool> _updatePaymentInDB() async{
    ...
    try{
      Pay pay = Pay(
          ...
          amount: _amount,
          payment: _paymentController.text,
          confirming: int.parse(confirming.isNotEmpty
              ? confirming
              : '0' ),
  ...
  Future<File> _localFile(File inputFile) async {
    ...
    final encryptedName  = uuid.v5(Uuid.NAMESPACE_URL,
        '${DateTime.now().toString()}$id$originalName');
